...
make_totals_mean_CI_compare <- function(
  ...
  outfile1 <- "FigI.png"
  ggsave(outfile1, p, width = width, height = height, dpi = dpi, bg = "white")
  message("Saved totals: ", outfile1)
  
  invisible(list(plot = p, outfile = outfile1, ci = ci_all, colors = scen_cols))
}


base_dirs <- (c("mgdriveHomming_ideal_v2","mgdriveHomming_non_ideal_v3", "mgdriveXShredder_ideal_v2", "mgdriveXShredder_non_ideal_v2", "mgdriveYLE_rel_5per_250", "mgdriveYLE_funR_rel_5per_250"))
labels_tag <- c("Ideal HGD","Non-ideal HGD", "Ideal X-Shredder","Non-ideal X-Shredder", "YLE (non-func.)", "YLE (func.)")
col_tags <-c("#377eb8", "#4daf4a", "#984ea3", "#ff7f00","#e41a1c","#a65628")
...
make_totals_mean_CI_compare(
  ...
  center    = "mean",
  tEnd      = 25,
  legpos_tag = c(0.48,0.55),
  colors    = col_tags,  # <-- user palette
  alpha_ci = 0.20, lw_mean = 1.8,
