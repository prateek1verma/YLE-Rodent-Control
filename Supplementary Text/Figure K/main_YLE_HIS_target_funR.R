...
####################
outFolder   <- "mgdriveYLE_HIS_rel_06per_fy0p8_pq_0p33_functionalR"
tMax        <- 58 * 365              # simulation time (days)
nRep        <- 2                    # number of Monte Carlo iterations
sitesNumber <- 1                     # <<< choose any number of patches
...
####################
if (sitesNumber == 1) {
  moveMat <- matrix(1, nrow = 1, ncol = 1)  # no dispersal
} else {
  moveMat <- triDiag(
...
####################
# source("generate_YLE_inheritance_cube_nofunR.R")
source("generate_YLE_inheritance_cube_funR.R")
pq <- 1/3
fl    <- 0
fs    <- 1
mu_val <- 0.97
...
cube <- generate_YLE_inheritance_cube_funR(
...
####################
netPar <- MGDrivEmouse2::parameterizeMGDrivE(
  runID          = 1,
  simTime        = tMax,
  nPatch         = sitesNumber,
...
MGDrivEmouse2::setupMGDrivE(stochasticityON = TRUE, verbose = FALSE)

MGDrivESim <- MGDrivEmouse2::Network$new(
  params          = netPar,
  driveCube       = cube,
