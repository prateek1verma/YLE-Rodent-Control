...
make_patch_figure_multi_compare <- function(
  ...
  if (y_log) {
    ...
    ymin <- min(sumci$lower, na.rm = TRUE)
    p <- p + coord_cartesian(ylim = c(9000, NA))
  }
  ...
  if (is.null(outfile)) {
    outfile <- file.path(
      normalizePath(getwd()),
      sprintf(
        "FIG_H_compare_TotalPop_Patch%s_t%s_%s_%s.png",
...
base_dirs <- c(
  "rel0p05_fy1p00_pq0p00_fl0p00_fs1p00_mu0p97_j0p25_c0p93"
)

labels <- c("250 YLE/month")

scenario_cols <- c("#006A8EFF") # c("#BB9753FF","#061922FF","#008348FF") # c("#252525","#CC79A7","#009E73") # Green: "#009E73" # "#525252" "#252525"
