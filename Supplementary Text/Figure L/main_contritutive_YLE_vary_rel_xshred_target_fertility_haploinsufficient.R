...
suppressPackageStartupMessages({
  library(MGDrivEmouse2) # This is from MGDrivEmouse2 package and not the MGDriVE for mosquitoes.
  # library(MGDriVE)
  library(BBmisc)
  library(parallel)
  library(pbapply)   
...
# Top-level output
####################
outFolder <- "mgdriveYLEboosted_constritutive_sweep_10yr_release_target_fertility_sweep"
...
############################
tMax  <- 58 * 365       # days
# nRep = 100 runs in the data provided in the paper
...
rel_grid   <- seq(0.002, 0.04, by = 0.002) # c(0.001, 0.002, 0.005, 0.010, 0.02, 0.04) # seq(0.01, 0.20, by = 0.01)
...
j_val  <- 0.25
c_val  <- 0.93
x_shred_grid <- c(0.8,0.5) # seq(0, 1, by = 0.1)
####################
...
build_tag <- function(rel, fy, pq, fl, fs, mu, j, c, x_shred) {
  paste0(
    "rel", num_tag(rel),
    "_fy", num_tag(fy),
    "_pq", num_tag(pq),
...
run_param_set <- function(rel_prop, fy, pq, fl, fs, x_shred) {
  # ===== inheritance cube for this param-set =====
  # alleleFitness_vec <- c(W = 1, H = 1, R = 1, Y = 1, X = 1, y = fy)
  alleleFitness_vec <- c(W = 1, H = 1, R = 1, Y = 1, X = 1, y = fy, A = 0.8, B = 1)
  ...
  netPar <- parameterizeMGDrivE(
    ...
    AdPopRatio_M = matrix(c(1, 0), 1, 2, dimnames = list(NULL, c("mYWWBB", "fXWWBB"))),
    AdPopRatio_F = matrix(c(0, 1), 1, 2, dimnames = list(NULL, c("mYWWBB", "fXWWBB")))
  )
  
  # # Define initial population ratios for 54 genotypes
  # # Initialize with 0s for all genotypes
  # init_males <- setNames(numeric(cube$genotypesN), cube$genotypesID)
  # init_females <- setNames(numeric(cube$genotypesN), cube$genotypesID)
  # 
  # # Set Wild-Type to 1 (mYWWBB and fXWWBB)
  # init_males["mYWWBB"] <- 1
  # init_females["fXWWBB"] <- 1
  # 
  # # Convert to the matrix format MGDrivE expects (nPatch x nGenotypes)
  # AdPopRatio_M_mat <- matrix(init_males, nrow = sitesNumber, byrow = TRUE)
  # AdPopRatio_F_mat <- matrix(init_females, nrow = sitesNumber, byrow = TRUE)
  # 
  ...
  #   beta = bioParameters$betaK, litters = bioParameters$litters,
  #   tGest = bioParameters$tGest, tNursing = bioParameters$tNursing, tAdo = bioParameters$tAdo,
  #   k = cap, theta = bioParameters$theta, 
  #   inheritanceCube = cube,
  #   AdPopRatio_M = AdPopRatio_M_mat,
  #   AdPopRatio_F = AdPopRatio_F_mat
  # )
  
  # ===== folders: mgdriveYLE_sweep/<tag>/<rep> =====
  tag <- build_tag(rel_prop, fy, pq, fl, fs, mu_val, j_val, c_val, x_shred)
...
cores <- if (is_windows) 1 else round(max(1L, parallel::detectCores()-2))
...
if (cores > 1) {
  invisible(pbmclapply(
    ...
    FUN = function(ii) {
      with(param_grid[ii, ], run_param_set(rel, fy, pq, fl, fs, x_shred))
    },
    mc.cores = cores
  ))
} else {
