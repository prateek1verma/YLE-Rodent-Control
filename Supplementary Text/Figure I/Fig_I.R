...
scenario_info <- tibble::tribble(
  ...
  "SMR",     "mgdriveSIT_sweep_10yr_small_rel",      1,        "Release size (% of adult sterile males released)",
  "fsRRDL",  "mgdrive_fsRIDL_10yrs",                  1,        "Release size (% of adult fsRRDL males released)",
  "YLE",     "mgdrive_YLE_10yrs", NA, "Release size (% of adult YLE males released)"
)
...
out_file_png <- "FigI.png"
out_file_pdf <- "FigI.pdf"
...
make_metric_plot <- function(metric_name, ylab, y_limits = NULL) {
  
  plot_df <- summ_tbl %>%
    filter(metric == metric_name) %>%
    mutate(strategy = factor(strategy, levels = c("YLE", "fsRRDL", "SMR")))
  ...
  ggplot(plot_df, aes(x = rel_pct, y = mean,
    ...
    scale_shape_manual(values = c("YLE" = 16, "fsRRDL" = 17, "SMR" = 15)) +
