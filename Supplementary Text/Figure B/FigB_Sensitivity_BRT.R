...
root <- "mgdriveYLE_sweep_Latin4_fix_rel_3per"           # << change only if your root differs
...
make_stratified_folds <- function(y, k = 5L) {
  ...
  if (n1 == 0L || n0 == 0L) return(list(folds = NULL, k = 0L))     # all one class
  k_eff <- min(k, n1, n0)                                         # ensure both classes in each fold
  if (k_eff < 2L) return(list(folds = NULL, k = 0L))              # too few minority samples
  
  folds <- integer(length(y))
  set.seed(1)
  folds[y == 1L] <- sample(rep(seq_len(k_eff), length.out = n1))
  folds[y == 0L] <- sample(rep(seq_len(k_eff), length.out = n0))
  list(folds = folds, k = k_eff)
}

safe_summary_gbm <- function(gbm_obj) {
  nbest <- tryCatch({
    bt <- gbm_obj$gbm.call$best.trees
    if (is.null(bt) || !is.finite(bt) || bt < 1) gbm::gbm.perf(gbm_obj, method = "cv", plot.it = FALSE) else bt
  }, error = function(e) NA_real_)
  if (!is.finite(nbest) || nbest < 1) return(NULL)
  gbm::summary.gbm(gbm_obj, n.trees = as.integer(nbest), plotit = FALSE)
}
...
keep_cols <- intersect(selected_params, c("fy","p","q","fs","mu","j","c"))
...
# (gbm.step, per paper)
# ===============================
# --- BINOMIAL BRT (probability) with robust folds ---
mod_prob <- dataset %>%
  dplyr::select(elim, all_of(keep_cols)) %>%
  dplyr::mutate(elim = as.integer(elim))

prob_prep <- drop_nzv_and_build_x(mod_prob, "elim")  # your existing utility (returns base data.frame)

y <- prob_prep$data[ , prob_prep$gbm.y]
fold_info <- make_stratified_folds(y, k = 5L)

table(prob_prep$data$elim)
prop.table(table(prob_prep$data$elim))
sapply(prob_prep$data[ , prob_prep$gbm.x, drop=FALSE], sd)  # confirm non-zero variance
...
if (!is.null(fold_info$folds)) {
  gbm_prob <- dismo::gbm.step(
    ...
    bag.fraction    = 0.75,
    n.folds         = fold_info$k,
    fold.vector     = fold_info$folds,  # <-- enforce class-balanced folds
    verbose         = FALSE
  )
} else {
  message("Bernoulli CV not feasible (extreme prevalence). Fitting without CV.")
  gbm_prob <- dismo::gbm.step(
    data            = prob_prep$data,
    gbm.x           = prob_prep$gbm.x,
    gbm.y           = prob_prep$gbm.y,
    ...
    learning.rate   = 0.01,
    bag.fraction    = 0.75,
    n.folds         = 0,        # disable CV to avoid ROC/threshold stats
    verbose         = FALSE
  )
}

sm_prob <- safe_summary_gbm(gbm_prob)
if (is.null(sm_prob)) stop("Failed to compute summary for probability model (check class balance).")

imp_prob <- tibble::as_tibble(sm_prob) |>
...
expression_map <- c(
  # "rel" = "Release Size",
  "fy"  = expression(paste("Male Fitness (", italic(f)[y], ")")),
  ...
  "mu"  = expression(paste("Mutation Rate (", italic(mu), ")")),
...
p <- ggplot(imp_all, aes(x = sign_val, y = param, fill = outcome)) +
  ...
  scale_x_continuous(
    ...
    breaks = seq(-max_abs, max_abs, by = 20),
...
readr::write_csv(agg, file.path(root, "agg_for_reference.csv"))
