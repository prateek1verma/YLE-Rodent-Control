...
############################
tMax  <- 58 * 365       # days
nRep  <- 20            # Monte Carlo iterations per parameter set
...
rel_grid   <- seq(0.01, 0.10, by = 0.01)
...
run_param_set <- function(rel_prop, fy, pq, fl) {
  ...
  alleleFitness_vec <- c(W = 1, H = 1, R = 1, Y = 1, X = 1, y = fy_val)
...
cores <- if (is_windows) 1 else max(1L, parallel::detectCores() - 2L)
