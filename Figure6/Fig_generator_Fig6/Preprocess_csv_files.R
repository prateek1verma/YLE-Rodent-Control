...
root <- "mgdriveYLEboosted_constritutive_sweep_10yr_release_target_fertility_sweep"
...
parse_param_tags <- function(tag,
  ...
  vals <- list(
    ...
    xs      = parse_num_from_tag(tag, "xs")
  )
  
  # allow either xs or x_shred naming
  if (is.na(vals$xs)) vals$xs <- parse_num_from_tag(tag, "x_shred")
...
run_metrics <- function(run_dir, fem_thresh, release_start_day, days_per_year, Neq, equil_window_days) {
  ...
  TS <- tibble(
    Time = fdf$Time,
    female_total = as.numeric(female_total)
  ) %>%
    inner_join(
      tibble(
        Time = mdf$Time,
        male_total = as.numeric(male_total)
      ),
      by = "Time"
    ) %>%
    arrange(Time) %>%
...
summ_tbl <- per_run_tbl %>%
  ...
  group_by(tag, rel, x_shred, fy, pq, fl, fs, mu, j, c) %>%
  summarise(
    n_runs = n(),
  ...
  mutate(
    rel_pct     = 100 * rel,
    x_shred_pct = x_shred,
    rel_lab     = sprintf("%g", rel_pct),
    x_shred_lab = sprintf("%g", x_shred_pct)
  )
...
filtered_tbl <- summ_tbl %>%
  filter(is.finite(rel), is.finite(x_shred), n_runs > 0)
...
x_shred_filter_pct <- NULL  # e.g. c(0, 100)

if (!is.null(x_shred_filter_pct)) {
  stopifnot(length(x_shred_filter_pct) == 2)
  filtered_tbl <- filtered_tbl %>%
    filter(x_shred_pct >= min(x_shred_filter_pct),
           x_shred_pct <= max(x_shred_filter_pct))
}
stopifnot(nrow(filtered_tbl) > 0)

# Ordered factor labels and integer grid coordinates
rel_levels <- filtered_tbl %>%
  distinct(rel_pct, rel_lab) %>%
  arrange(rel_pct) %>%
  pull(rel_lab)
x_break_df <- filtered_tbl %>%
  distinct(rel_pct, rel_lab) %>%
  arrange(rel_pct) %>%
  mutate(x_idx = row_number()) %>%
  filter(rel_pct >= 0.2, rel_pct <= 2.0,
         abs((rel_pct / 0.2) - round(rel_pct / 0.2)) < 1e-8)

x_shred_levels <- filtered_tbl %>%
  distinct(x_shred_pct, x_shred_lab) %>%
  arrange(x_shred_pct) %>%
  pull(x_shred_lab)
...
filtered_tbl <- filtered_tbl %>%
  mutate(
    ...
    y_idx = as.integer(factor(x_shred_lab, levels = x_shred_levels))
  )
...
if (!is.null(tte_highlight_range_years)) {
  ...
  grid_df <- tidyr::expand_grid(rel_lab = rel_levels, x_shred_lab = x_shred_levels) %>%
    left_join(filtered_tbl, by = c("rel_lab", "x_shred_lab")) %>%
    mutate(
      x_idx = as.integer(factor(rel_lab, levels = rel_levels)),
      y_idx = as.integer(factor(x_shred_lab, levels = x_shred_levels)),
...
nx <- length(rel_levels); ny <- length(x_shred_levels)
...
pA <- ggplot(filtered_tbl, aes(x = x_idx, y = y_idx, fill = A_prob_elim)) +
  ...
  scale_x_continuous( expand = c(0, 0), breaks = x_break_df$x_idx, labels = x_break_df$rel_lab ) +
  scale_y_continuous(expand = c(0, 0), breaks = seq_along(x_shred_levels), labels = x_shred_levels) +
  scale_fill_gradient(
...
summary_param <- summ_tbl %>%
  transmute(
    tag, rel, x_shred, fy, pq, fl, fs, mu, j, c,
    rel_pct = round(rel_pct, 2),
    x_shred_pct = round(x_shred_pct, 2),
    n_runs,
...
summary_filtered <- filtered_tbl %>%
  select(tag, rel, x_shred, fy, pq, fl, fs, mu, j, c,
         rel_pct, x_shred_pct, rel_lab, x_shred_lab, x_idx, y_idx,
         n_runs,
         A_prob_elim,
