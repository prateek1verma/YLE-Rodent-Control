...
plot_nonwt_multidesign_by_patch <- function(
  ...
  y_lab <- if (mode == "timepoint") {
    paste0("Mean transgenic carrier abundance\n(t = ", t_years, " years)")
  } else {
    "Mean peak transgenic \n carrier abundance"
  }
  ...
  if (is.null(outfile)) {
    ...
    outfile <- file.path(path.expand(getwd()),
                         paste0("Figure6b", suffix, ".png"))
    outfile2 <- file.path(path.expand(getwd()),
                         paste0("Figure6b", suffix, ".pdf"))
  }
  
  ggsave(outfile, p, width = 10, height = 8, dpi = 600, bg = "white")
  ggsave(outfile2, p, width = 10, height = 8, dpi = 600, device = cairo_pdf)
  message("Saved plot: ", outfile)
  message("Saved plot: ", outfile2)
...
base_dirs <- c("mgdriveYLE_rel_5per_250_5patches_mig_2daily",
               "mgdrive_boosted_YLE_rel_1per_5patches_mig_2daily_fy0p8_xshred_0p80",
               "mgdrive_boosted_YLE_rel_2per_5patches_mig_2daily_fy0p8_xshred_0p80",
               "mgdrive_boosted_YLE_rel_2per_5patches_mig_2daily_fy0p8_xshred_0p50")

# labels_tag <- c("YLE (5% release)", "Boosted (1%, XS=80%)", "Boosted (2%, XS=80%)", "Boosted (2%, XS=50%)")
labels_tag <- c(
  "YLE (5%)",
  "Boosted (1%, XS=80%)",
  "Boosted (2%, XS=80%)",
...
# col_tags <- c("#d7191c", "#deebf7", "#9ecae1","#3182bd")
col_tags <- c("#d7191c", "#99d594", "#ffffbf","#fc8d59")
...
genotypes <- c(
  "mYWWAA","mYWWAB","mYWWBB","mYHWAA","mYHWAB","mYHWBB","mYRWAA","mYRWAB","mYRWBB",
  "mYHHAA","mYHHAB","mYHHBB","mYHRAA","mYHRAB","mYHRBB","mYRRAA","mYRRAB","mYRRBB",
  "myWWAA","myWWAB","myWWBB","myHWAA","myHWAB","myHWBB","myRWAA","myRWAB","myRWBB",
  "myHHAA","myHHAB","myHHBB","myHRAA","myHRAB","myHRBB","myRRAA","myRRAB","myRRBB",
  "fXWWAA","fXWWAB","fXWWBB","fXHWAA","fXHWAB","fXHWBB","fXRWAA","fXRWAB","fXRWBB",
  "fXHHAA","fXHHAB","fXHHBB","fXHRAA","fXHRAB","fXHRBB","fXRRAA","fXRRAB","fXRRBB"
)

transgenic_genotype <- genotypes[grepl("y|A", genotypes)]


genotype_map <- list(
  "YLE (5%)" = c("myWW","myHW","myRW","myHH","myHR","myRR"),
  "Boosted (1%, XS=80%)"   = transgenic_genotype,
  "Boosted (2%, XS=80%)"   = transgenic_genotype,
  "Boosted (2%, XS=50%)" = transgenic_genotype
)
