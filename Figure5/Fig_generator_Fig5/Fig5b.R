...
make_totals_mean_CI_compare <- function(
  ...
  scen_cols <- .map_colors_to_labels(labels, colors = colors, lighten_amts = lighten_amts)
  
  base_theme <- theme_bw(base_size = 32) +
    theme(
      ...
      legend.background    = element_rect(
        fill   = scales::alpha("white", 1),  # <-- slightly transparent
        colour = "white"
      ),
  ...
  if (!show_legend) {
    base_theme <- base_theme + theme(legend.position = "none")
  }
...
base_dirs <- c("mgdrive_Homing_5patch_250_mig_1daily","mgdrive_Homing_non_ideal_5patch_250_mig_1daily","mgdrive_Xshredder_ideal_5patch_250_mig_1daily","mgdrive_Xshredder_nonideal_5patch_250_mig_1daily", "mgdriveYLE_rel_5per_250_5patches_mig_1daily")
labels_tag <- c("Ideal HGD","Non-ideal HGD", "Ideal X-Shredder","Non-ideal X-Shredder", "250 YLE/month")
#  c("Ideal HGD","Non-ideal HGD", "250 YLE/month") # "Ideal X-Shredder","Non-ideal X-Shredder", 
col_tags <- c("#377eb8", "#4daf4a", "#984ea3", "#ff7f00","#e41a1c")
