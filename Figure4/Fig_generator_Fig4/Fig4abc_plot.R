...
roots <- c(
  "mgdrivefsRRDL_sweep_10yrs_heatmap"
  # , "another_run_folder", "third_run_folder", ...
)
...
legend_h_cm  <- 0.7 # 0.5    # horizontal bar height (cm)
legend_w_cm  <- max(4, nx * 0.62)  # horizontal bar width (cm)
panel_pad_cm <- 2.0
title_axis_pad <- 10.0
...
# x ticks: 4, 8, 12, ... up to max rel (e.g., 40)
  rel_levels_num <- suppressWarnings(as.numeric(as.character(rel_levels)))
  tick_vals <- seq(4, max(rel_levels_num, na.rm = TRUE), by = 4)

  x_map <- grid_plot |>
    dplyr::distinct(rel_lab, x_idx) |>
    dplyr::mutate(rel_lab_num = suppressWarnings(as.numeric(as.character(rel_lab)))) |>
    dplyr::filter(rel_lab_num %in% tick_vals) |>
    dplyr::arrange(rel_lab_num)

  new_breaks <- x_map$x_idx
  new_labels <- as.character(x_map$rel_lab_num)
  ...
  pA <- ggplot(grid_plot, aes(x = x_idx, y = y_idx, fill = A_prob_elim)) +
    ...
    labs(
      ...
      x = "% of released fsRRDL males\nat K (per month)",
      y = "% of life-span reduction in fsRRDL\nmales relative to WT"
    ) +
  ...
  pB <- ggplot(grid_plot, aes(x = x_idx, y = y_idx, fill = tte_to_plot)) +
    ...
    ggtext::geom_richtext(
      x = 0.5 + 0.80*nx, y = 0.5 + 0.25*ny,
      label = "Median TTE<br>&le; 5 yrs", fill = NA, label.color = NA, size = 9
    )
...
colA <- apply_columnwise_tidy(
  ...
  xlab_for_bottom = "% of released fsRRDL males\nat K (per month)"
)
colB <- apply_columnwise_tidy(
  colB,
  title_for_top   = "Median time to elimination of\nfemale population (when PoE>0.5)",
  xlab_for_bottom = "% of released fsRRDL males\nat K (per month)"
)
