generate_YLE_inheritance_cube <- function(c = 0.93, j = 0.043, mu = 1, p = 0, q = 0, female_lethality = 1, alleleFitness = NULL) {
  # h <- 0.99
  # q <- 1
  # alleleFitness <- NULL
  
  if (is.null(alleleFitness)) {
    # alleleFitness <- c(W = 1, H = 0.99, R = 1)
    alleleFitness <- c(W = 1, H = 0.99, R = 1, Y=1, X=1, y=1)
  }
  ...
  lethal_genos_f <- c("fXHW", "fXHH", "fXHR", "fXRR") # c("fXHH", "fXRR") # c("fXHW", "fXHH", "fXHR","fXRR")
...
# cube <- generate_YLE_inheritance_cube(c = 1, j = 0, mu = 1, p = 0, q = 0, alleleFitness = NULL)
