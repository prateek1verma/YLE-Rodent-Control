...
# Totals (males + females) for *multiple* runs
build_total_series_multi <- function(base_dir, run_ids, patch_id, tstart) {
  purrr::map_dfr(run_ids, function(rid) {
    built <- build_series_long(base_dir, rid, patch_id, tstart)
    built$data_joined %>%
      transmute(
...
# -----------------------------------------------------#
make_patch_figure_multi_tot <- function(
    ...
    lw_mean    = 0.8,       # linewidth for mean line (group means)
    y_pad      = 0.06,      # headroom added on top of auto y max
    trim_upper = 0          # set e.g. 0.01 to trim at 99th pct for spikes
) {
  if (is.null(run_ids)) run_ids <- list_run_ids(base_dir)
  
  # Load & stack all runs
  all_series   <- build_series_long_multi(base_dir, run_ids, patch_id, tstart)
  total_series <- build_total_series_multi(base_dir, run_ids, patch_id, tstart)
  ...
  mean_total <- total_series %>%
    dplyr::group_by(Years) %>%
    dplyr::summarize(mean_total = mean(total, na.rm = TRUE), .groups = "drop")
  
  # X domain (years)
  x_lim <- if (is.null(tEnd)) range(all_series$Years, na.rm = TRUE) else c(0, tEnd)
  ...
  okabe_ito <- c(
    "female total" = "#7F7F7F",  # grey
    "male Y"       = "#1F77B4",  # blue
    "male y"       = "#D55E00"   # orange-red
  )
  
  # ---------- Right axis is 2× left ----------
  right_factor <- 2
  a <- 1 / right_factor      # totals are plotted as (total * a) on left; right shows true totals
  
  # Auto y-upper so **everything** is visible on left (groups + scaled totals)
  left_vals_groups <- all_series %>%
    dplyr::filter(Years >= x_lim[1], Years <= x_lim[2]) %>%
    dplyr::pull(count)
  
  left_vals_totals <- total_series %>%
    dplyr::filter(Years >= x_lim[1], Years <= x_lim[2]) %>%
    dplyr::transmute(val = total * a) %>%
    dplyr::pull(val)
  
  vals <- c(left_vals_groups, left_vals_totals)
  if (trim_upper > 0 && is.finite(trim_upper) && trim_upper > 0 && trim_upper < 1) {
    ymax_base <- stats::quantile(vals, probs = 1 - trim_upper, na.rm = TRUE)
  } else {
    ymax_base <- suppressWarnings(max(vals, na.rm = TRUE))
  }
  if (!is.finite(ymax_base) || ymax_base <= 0) ymax_base <- 1
  y_upper <- (1 + y_pad) * ymax_base
  
  # Build plot
  p <- ggplot() +
    # Many runs (thin, translucent)
    geom_line(
      data = dplyr::filter(all_series, Years >= x_lim[1], Years <= x_lim[2]),
      aes(x = Years, y = count, color = group, group = interaction(run_id, group)),
      linewidth = lw_many, alpha = alpha_many
    ) +
    # Mean per group (thicker)
    geom_line(
      data = dplyr::filter(mean_series, Years >= x_lim[1], Years <= x_lim[2]),
      aes(x = Years, y = mean_count, color = group),
      linewidth = lw_mean, alpha = 0.9
    ) +
    # Totals (mean) plotted on left after scaling by 'a'; right axis will show true totals
    geom_line(
      data = dplyr::filter(mean_total, Years >= x_lim[1], Years <= x_lim[2]),
      aes(x = Years, y = mean_total * a),
      color = "grey10", linewidth = lw_mean, show.legend = FALSE
    ) +
    labs(
  ...
  if (y_log) {
    # For log scale, we skip secondary axis (interpretation issues)
    p <- p + scale_y_log10(labels = scales::label_number(big.mark = ""))
    message("Note: Right=2×left alignment is applied only for linear scale; secondary axis omitted for y_log=TRUE.")
  } else {
    p <- p + scale_y_continuous(
      labels = scales::label_number(big.mark = ""),
      breaks = scales::breaks_pretty(n = 6)(c(0, y_upper)),
      limits = c(0, y_upper),
      sec.axis = sec_axis(~ . / a, name = "Total population")
    )
  }
  ...
  if (is.null(outfile)) {
    outfile <- file.path(
      ...
      sprintf("Figure_Patch%s_MULTI_t%s_tot.png", patch_id, tstart)
    )
  }
  ...
  invisible(list(
    ...
    left_to_right_factor = right_factor,  # = 2
    totals_left_scale = a,                # = 0.5
    y_upper = y_upper
  ))
}
...
#   base_dir = "~/Library/CloudStorage/Box-Box/Research Projects/Rodent GD/MGDrivE-master/ExampleCode/mgdriveYLE",
#   patch_id = "001",
#   tstart   = round(3000-365*2),
#   y_log    = FALSE
# )
