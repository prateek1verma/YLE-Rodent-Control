...
roots <- c(
  "mgdriveYLE_sweep_10yr_release_target_fertility_suff"
  , "mgdriveYLE_sweep_10yr_release_target_fertility_partial_insuff"
  # ,"mgdriveYLE_sweep_10yr_release_viability_partial_insuff"
...
legend_w_cm  <- max(4, nx * 0.6)  # horizontal bar width (cm)
panel_pad_cm <- 2.0
title_axis_pad <- 4.0
...
png_path <- file.path(out_root, "Fig3.png")
pdf_path <- file.path(out_root, "Fig3.pdf")


ggsave("Fig3.png", plot = combined, width = w_total_cm, height = h_total_cm,
       units = "cm", dpi = 300, bg = "white")

auto_crop_png(png_path, fuzz = 12)
ggsave("Fig3.pdf", plot = combined, device = "pdf", width = w_total_cm, height = h_total_cm,
       units = "cm", bg = "white")
